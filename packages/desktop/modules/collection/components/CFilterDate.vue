...
const operation = computed<keyof typeof operations.date>({
    get() {
...
set(model.value, 'config.format', props.column.displayFormat)

const options = Object.keys(operations.date).map((key) => ({
    label: key,
    value: key,
}))
</script>
